...
void PixelPaintView::DrawBrushSettings()
{
    ...
    ImGui::Checkbox("Eraser Alpha##er_alpha", &eraserUseAlpha);
    ImGui::SetItemTooltip("Erase to transparent (alpha 0) instead of a solid color");

    if (!eraserUseAlpha) {
        ImGui::SameLine();
        ImVec4 erCol = ImVec4(eraserColor.r/255.0f, eraserColor.g/255.0f, eraserColor.b/255.0f, eraserColor.a/255.0f);
        if (ImGui::ColorButton("Eraser Color##er_btn", erCol)) {
            ImGui::OpenPopup("EraserColorPicker");
        }
        if (ImGui::BeginPopup("EraserColorPicker")) {
            float col[3] = { erCol.x, erCol.y, erCol.z };
            if (ImGui::ColorPicker3("##er_picker", col)) {
                eraserColor.r = (uint8_t)(col[0]*255);
                eraserColor.g = (uint8_t)(col[1]*255);
                eraserColor.b = (uint8_t)(col[2]*255);
            }
            ImGui::EndPopup();
        }
    }
    ...
    ImGui::Checkbox("Bucket Alpha##bk_alpha", &bucketEraseToAlpha);
